#!/bin/bash
#TIV will generally by stored under /report in an XML. 
#This will get TIV for every file in the report.
...
for file in $NIFTI_PATH; do
  ...
  singularity run --cleanenv --bind "$FILE_DIR":/mnt cat-r1853.simg \
    -b /batch/cat_standalone_get_TIV.m \
    /mnt/cat_*.xml  -a1 "'TIV.txt'"
done
